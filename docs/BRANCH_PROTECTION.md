...
- Require status checks to pass before merging
  - Add required checks: `API CI` (from .github/workflows/api.yml) and `Web CI` (from .github/workflows/web.yml)
...
3. Check "Require status checks to pass before merging" and select the CI checks from the list (after workflows have run at least once the checks will appear)
